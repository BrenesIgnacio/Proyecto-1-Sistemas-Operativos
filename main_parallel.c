...
int parallel_decompress(const char *archive_file) {
    ...
    while (file_count < MAX_FILES) {
        ...
        if (fread(file_info[file_count].filename, 1, name_len, archive) != name_len) break;
        file_info[file_count].filename[name_len] = '\0';
        
        if (fread(&file_info[file_count].size, sizeof(uint32_t), 1, archive) != 1) break;
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        // Comprimiendo archivos en paralelo (silencioso)
    ...
    // Opciones de línea de comandos (mismo que antes)
    if (argc < 3) {
